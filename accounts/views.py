...
from journalApp.models import Article_Journal, Journal
...
class LogInView(GuestOnlyView, FormView):
    ...
    def form_valid(self, form):
        ...
        messages.success(request, _('Welcome again') + " " + request.user.first_name)
...
class SignUpView(GuestOnlyView, FormView):
    ...
    def form_valid(self, form):
        ...
        if settings.ENABLE_USER_ACTIVATION:
            ...
            messages.success(
                request, _('We have received your registration, please click on the link sent for activation'))
        else:
            ...
            messages.success(request, _('Your Registration is Complete, Welcome to Us'))
...
class ResendActivationCodeView(GuestOnlyView, FormView):
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, _('New Activation Code Sent To Your E-Mail Address'))
    ...
    def form_valid(self, form):
        ...
        def msg():
            ...
            for i in form.fields:
                if form.initial[i] == form.cleaned_data.get(i, None) or form.cleaned_data.get(i, None):
                    exp.append("T")
                else:
                    exp.append("F")
            return exp
        ...
        if "F" in msg():
            ...
            messages.success(self.request, _('Anything Changed :)'))
...
class ChangeEmailView(LoginRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        if settings.ENABLE_ACTIVATION_AFTER_EMAIL_CHANGE:
            ...
            messages.success(self.request, _('Please, Click On The Link We Sent For E-Mail Exchange'))
        else:
            user.email = email
            user.save()
...
class ChangeEmailActivateView(View):
    ...
    def get(request, code):
        ...
        messages.success(request, _('Your E-Mail Address Has Been Successfully Changed'))
...
class RemindUsernameView(GuestOnlyView, FormView):
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, _('Your User Name Has Been Sent To Your E-Mail Address.'))
...
class RestorePasswordConfirmView(BasePasswordResetConfirmView):
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, _('Your Password Has Been Set, you can login now.'))
...
def articles_journals_of_user(request, username):
    if request.user.is_authenticated:
        user = CustomUser.objects.filter(username=username).first()
        user_articles = Article_Journal.objects.filter(author_of_article=user)
        user_journals = Journal.objects.filter(journal_editor=user)
        ...
        return render(request, "accounts/profile/userprofile.html",
                      context={"user": user, 'user_journals': user_journals,
                               "user_articles": user_articles, "requestable": requestable_journals})
    else:
        messages.success(request, _('Please Login or Register to View the Page!'),
                         extra_tags='danger')
